...
const Banner = () => {
  ...
  const fetchData = async () => {
    // 현재 상영중인 영화정보를 가져옴 async
    const response = await axios.get(requests.fetchNowPlaying); // https://api.themoviedb.org/3/movie/now_playing 같은 url
    // console.log("request: ", response);

    // 영화 배열 중에 하나의 ID를 가져옴 (+랜덤)
    const movieId = response.data.results[Math.floor(Math.random() * response.data.results.length)].id;
    // console.log("movieId: ", movieId);

    // 특정 영화의 디테일 정보를 가져옴 (+비디오 정보)
